...
// general ui settings
//#############################################################
const TRAFFIC_LIGHT_GREEN_TIME = 10; // seconds
const TRAFFIC_LIGHT_YELLOW_TIME = 3; // seconds
const TRAFFIC_LIGHT_RED_TIME = 10;   // seconds
...
function updateTrafficLightsAuto() {
    console.log("updateTrafficLightsAuto called");
    if (!road || !road.trafficLights) {
        console.log("road or trafficLights not found");
        return;
    }

    for (let i = 0; i < road.trafficLights.length; i++) {
        const tl = road.trafficLights[i];
        console.log("Updating traffic light", tl.id);

        if (!trafficLightTimers[tl.id]) {
            trafficLightTimers[tl.id] = { state: 'green', timer: 0 };
            tl.green = true;
            tl.yellow = false;
            tl.red = false;
        }

        const light = trafficLightTimers[tl.id];
        light.timer += dt;

        if (light.state === 'green' && light.timer >= TRAFFIC_LIGHT_GREEN_TIME) {
            light.state = 'yellow';
            light.timer = 0;
            tl.green = false;
            tl.yellow = true;
            tl.red = false;
            console.log(`Light ${tl.id} changed to yellow`);
        } else if (light.state === 'yellow' && light.timer >= TRAFFIC_LIGHT_YELLOW_TIME) {
            light.state = 'red';
            light.timer = 0;
            tl.green = false;
            tl.yellow = false;
            tl.red = true;
            console.log(`Light ${tl.id} changed to red`);
        } else if (light.state === 'red' && light.timer >= TRAFFIC_LIGHT_RED_TIME) {
            light.state = 'green';
            light.timer = 0;
            tl.green = true;
            tl.yellow = false;
            tl.red = false;
            console.log(`Light ${tl.id} changed to green`);
        }
    }
}
